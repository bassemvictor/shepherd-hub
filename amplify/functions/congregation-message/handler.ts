...
type GoogleCalendarSyncSettingsPayload = {
...
type StoredGoogleCalendarSyncSettingsData = {
  syncBehavior?: "always" | "interval";
  syncIntervalMinutes?: number;
...
type ParsedImportMember = ParsedVcfContact & {
...
const decodeXmlEntities = (value: string) =>
  value.replace(/&(#x[0-9a-f]+|#\d+|amp|lt|gt|quot|apos|nbsp);/gi, (match, entity) => {
...
const buildImportedNotes = (items: Array<[string, string]>) =>
  items
    .filter(([, value]) => Boolean(value))
    .map(([label, value]) => `${label}: ${value}`)
    .join("\n");

const normalizeUnityMemberId = (value?: string) => {
  const cleaned = normalizeWhitespace(value);
...
const mergeAddressWithPostalCode = (address: string, postalCode: string) => {
  if (!postalCode) {
    return address;
  }
...
const getWorksheetColumnIndex = (cellReference: string) => {
...
const extractWorksheetCellValue = (cellContent: string, cellType?: string, sharedStrings: string[] = []) => {
  if (cellType === "inlineStr") {
    return decodeXmlEntities(
      Array.from(cellContent.matchAll(/<t\b[^>]*>([\s\S]*?)<\/t>/gi))
        .map((match) => match[1] ?? "")
        .join(""),
    );
  }
...
const parseWorksheetRows = (worksheetXml: string, sharedStrings: string[]) => {
  ...
  for (const rowMatch of worksheetXml.matchAll(/<row\b[^>]*>([\s\S]*?)<\/row>/gi)) {
    ...
    for (const cellMatch of rowContent.matchAll(/<c\b([^>]*?)(?:\/>|>([\s\S]*?)<\/c>)/gi)) {
      ...
      rowValues[columnIndex] = extractWorksheetCellValue(
        cellContent,
        cellType,
        sharedStrings,
      );
...
const unzipWorkbookEntries = (archiveBuffer: Buffer) => {
  const endOfCentralDirectorySignature = Buffer.from([0x50, 0x4b, 0x05, 0x06]);
  const endOfCentralDirectoryOffset = archiveBuffer.lastIndexOf(
  ...
  const centralDirectoryOffset = archiveBuffer.readUInt32LE(
  ...
  for (let index = 0; index < totalEntries; index += 1) {
    ...
    const fileName = archiveBuffer.toString(
      "utf8",
      fileNameStart,
...
const parseUnityExcelContacts = (contentBase64: string): ParsedImportMember[] => {
  ...
  const worksheetPath = Array.from(workbookEntries.keys()).find((entryName) =>
    /^xl\/worksheets\/[^/]+\.xml$/i.test(entryName),
  );
  ...
  const sharedStrings = parseSharedStrings(
    workbookEntries.get("xl/sharedStrings.xml")?.toString("utf8"),
  );
  const worksheetRows = parseWorksheetRows(
    workbookEntries.get(worksheetPath)?.toString("utf8") ?? "",
    sharedStrings,
  );
  ...
  const importedMembers: ParsedImportMember[] = [];
  ...
  for (const row of worksheetRows.slice(headerRowIndex + 1)) {
    ...
    const address = mergeAddressWithPostalCode(
      sanitizeImportedText(getRowValue(row, "Address")),
      postalCode,
    );
    const displayName =
      memberName || email || phone || `Unity member ${memberId}`;
...
const getContactsImportSourceType = (payload: Partial<ImportContactsPayload>) => {
...
const getDefaultGoogleCalendarSyncYearRange = (time: string) => {
...
const getGoogleCalendarSyncSettingsSortKey = (userKey: string) => `GOOGLE#${userKey}`;
...
const normalizeGoogleCalendarSyncIntervalMinutes = (value?: number) =>
  typeof value === "number" &&
...
const normalizeGoogleCalendarSyncYear = (value?: number) =>
  typeof value === "number" &&
  Number.isInteger(value) &&
  ...
  value <= maxGoogleCalendarSyncYear
    ? value
    : null;
const resolveGoogleCalendarSyncSettings = (
  settings: StoredGoogleCalendarSyncSettingsData | null | undefined,
  time: string,
): {
  syncBehavior: "always" | "interval";
  syncIntervalMinutes: number;
  ...
  const hasValidCustomRange =
    syncFromYear !== null && syncToYear !== null && syncFromYear <= syncToYear;

  return {
    syncBehavior: settings?.syncBehavior === "interval" ? "interval" : "always",
    syncIntervalMinutes: normalizeGoogleCalendarSyncIntervalMinutes(
      settings?.syncIntervalMinutes,
    ),
    cacheLastSyncAt:
      typeof settings?.cacheLastSyncAt === "string" &&
...
const getGoogleCalendarSyncWindow = (
  time: string,
  settings?: StoredGoogleCalendarSyncSettingsData | null,
) => {
  const resolvedSettings = resolveGoogleCalendarSyncSettings(settings, time);

  return {
    timeMin: new Date(
      Date.UTC(resolvedSettings.syncFromYear, 0, 1, 0, 0, 0, 0),
    ).toISOString(),
    timeMax: new Date(
      Date.UTC(resolvedSettings.syncToYear + 1, 0, 1, 0, 0, 0, 0),
    ).toISOString(),
  };
};
...
const loadGoogleCalendarEventsForCalendar = async ({
  ...
  let syncSettings: StoredGoogleCalendarSyncSettingsData | null | undefined;
...
const isGoogleCalendarEventInSyncWindow = (
  item: Pick<StoredGoogleCalendarSyncedEventData, "start" | "end">,
  syncWindow: { timeMin: string; timeMax: string },
) => {
...
const storeDirectGoogleCalendarEventsInCache = async ({
  ...
  timeMax: string;
  events: StoredGoogleCalendarSyncedEventData[];
  syncSettings?: StoredGoogleCalendarSyncSettingsData | null;
}) => {
...
const syncGoogleCalendarEventCache = async ({
  ...
  const cachedState = await loadAllGoogleCalendarCachedEvents(tableName, userKey, calendarId);
  const cachedEventsById = new Map<string, StoredGoogleCalendarSyncedEventData>(
  ...
  const boundedEvents = Array.from(cachedEventsById.values()).filter((item) =>
    isGoogleCalendarEventInSyncWindow(item, syncWindow),
  );
...
const getStoredGoogleCalendarSyncSettings = async (tableName: string, userKey: string) => {
  const response = await dynamoClient.send(
    new GetCommand({
      TableName: tableName,
      Key: {
  ...
  try {
    return JSON.parse(String(response.Item.data)) as StoredGoogleCalendarSyncSettingsData;
  } catch {
...
const saveStoredGoogleCalendarSyncSettings = async (
  tableName: string,
  userKey: string,
  settings: StoredGoogleCalendarSyncSettingsData,
) => {
  await dynamoClient.send(
    new PutCommand({
      TableName: tableName,
...
const deleteStoredGoogleCalendarConnection = async (tableName: string, userKey: string) => {
  await dynamoClient.send(
    new DeleteCommand({
      TableName: tableName,
      Key: {
...
const deleteStoredGoogleCalendarSyncSettings = async (tableName: string, userKey: string) => {
  await dynamoClient.send(
    new DeleteCommand({
      TableName: tableName,
      Key: {
...
const deleteStoredGoogleOauthState = async (tableName: string, state: string) => {
  await dynamoClient.send(
    new DeleteCommand({
      TableName: tableName,
      Key: {
...
const deleteStoredGoogleCalendarCachedRows = async (tableName: string, userKey: string) => {
  const response = await dynamoClient.send(
    new ScanCommand({
      TableName: tableName,
  ...
  await Promise.all(
    (((response.Items ?? []) as Array<Pick<TableRow, "pk" | "sk">>) ?? []).map((item) =>
      dynamoClient.send(
        new DeleteCommand({
          TableName: tableName,
          Key: {
...
const clearStoredGoogleCalendarCacheState = async (tableName: string, userKey: string, time: string) => {
...
const upsertStoredGoogleCalendarSyncSettings = async ({
  tableName,
  userKey,
  time,
  patch,
  ...
  const existingSettings = resolveGoogleCalendarSyncSettings(
    await getStoredGoogleCalendarSyncSettings(tableName, userKey),
    time,
  );
  const nextSettings: StoredGoogleCalendarSyncSettingsData = {
    syncBehavior:
      patch.syncBehavior === "interval" || patch.syncBehavior === "always"
        ? patch.syncBehavior
        : existingSettings.syncBehavior,
    syncIntervalMinutes:
      typeof patch.syncIntervalMinutes === "number"
        ? normalizeGoogleCalendarSyncIntervalMinutes(patch.syncIntervalMinutes)
        : existingSettings.syncIntervalMinutes,
    cacheLastSyncAt:
      patch.cacheLastSyncAt !== undefined
        ? patch.cacheLastSyncAt
        : existingSettings.cacheLastSyncAt,
    syncFromYear:
      typeof patch.syncFromYear === "number"
        ? patch.syncFromYear
        : existingSettings.syncFromYear,
    syncToYear:
      typeof patch.syncToYear === "number" ? patch.syncToYear : existingSettings.syncToYear,
    updatedAt: time,
  };
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  if (event.requestContext.http.method === "POST") {
    ...
    if (requestPath.endsWith("/calendar/google/connection/sync-settings")) {
      ...
      const payload = JSON.parse(event.body ?? "{}") as Partial<GoogleCalendarSyncSettingsPayload>;
      const syncBehavior =
        payload.syncBehavior === "interval" || payload.syncBehavior === "always"
          ? payload.syncBehavior
          : null;
      const syncIntervalMinutes = normalizeGoogleCalendarSyncIntervalMinutes(
        typeof payload.syncIntervalMinutes === "number" ? payload.syncIntervalMinutes : undefined,
      );
      const syncFromYear =
        payload.syncFromYear === undefined
          ? null
          : normalizeGoogleCalendarSyncYear(payload.syncFromYear);
      const syncToYear =
        payload.syncToYear === undefined
          ? null
      ...
      const existingSettings = resolveGoogleCalendarSyncSettings(
        await getStoredGoogleCalendarSyncSettings(tableName, requestUserKey),
        time,
      );
    ...
    if (requestPath.endsWith("/calendar/google/events/all")) {
      ...
      try {
        ...
        let syncSettings = resolveGoogleCalendarSyncSettings(
          await getStoredGoogleCalendarSyncSettings(tableName, requestUserKey),
          time,
        );

        if (useSyncCache && !cacheOnly && items.syncMode !== "cached") {
          syncSettings = resolveGoogleCalendarSyncSettings(
            await upsertStoredGoogleCalendarSyncSettings({
              tableName,
              userKey: requestUserKey,
              time,
    ...
    if (requestPath.endsWith("/contacts/import")) {
      ...
      let parsedContacts: ParsedImportMember[] = [];
      ...
      const memberSkKeys = new Set<string>();
          ...
          message:
            importedMembers.length > 0
              ? `Imported ${importedMembers.length} ${
                  sourceType === "unity-excel" ? "member" : "contact"
                }${
  ...
  if (requestPath.endsWith("/calendar/google/oauth/callback")) {
    ...
    if (requestPath.endsWith("/calendar/google/connection")) {
      ...
      const syncSettings = resolveGoogleCalendarSyncSettings(
        await getStoredGoogleCalendarSyncSettings(tableName, requestUserKey),
        time,
      );
      const existingConnection = await getStoredGoogleConnection(tableName, requestUserKey);
