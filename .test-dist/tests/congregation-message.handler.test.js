...
const getExpectedSyncWindow = ({ syncFromYear, syncToYear, } = {}) => {
...
const createEvent = ({ path, method = "GET", body, groups, email = "user@example.com", sub = "00000000-0000-4000-8000-000000000001", cognitoUsername = "user@example.com", headers, queryStringParameters, }) => ({
...
const createInlineStringCell = (cell, value) => `<c r="${cell}" t="inlineStr"><is><t>${value}</t></is></c>`;
const createNumberCell = (cell, value) => `<c r="${cell}"><v>${value}</v></c>`;
...
test("deletes an existing Google Calendar connection and cached rows", async () => {
    ...
    assert.deepEqual(dynamo.commands.map((command) => command.constructor.name), ["DeleteCommand", "DeleteCommand", "ScanCommand", "DeleteCommand", "DeleteCommand"]);
});
test("clears cached Google Calendar events and sync token without deleting the connection", async () => {
    const store = new Map();
    const keyFor = (pk, sk) => `${pk}||${sk}`;
    store.set(keyFor("CALENDAR_SYNC_SETTINGS", "GOOGLE#user@example.com"), {
    ...
    const body = parseBody(response.body);
    const savedSettings = JSON.parse(String(store.get(keyFor("CALENDAR_SYNC_SETTINGS", "GOOGLE#user@example.com"))?.data ?? "{}"));
    assert.equal(response.statusCode, 200);
    ...
    assert.deepEqual(dynamo.commands.map((command) => command.constructor.name), ["ScanCommand", "DeleteCommand", "DeleteCommand", "GetCommand", "PutCommand"]);
});
test("updates Google Calendar sync settings and exposes them from the connection endpoint", async () => {
    ...
    setHandlerClientsForTesting({ dynamoClient: dynamo.client });
    const saveResponse = await invokeHandler(createEvent({
    ...
    const savedSettings = JSON.parse(String(store.get(keyFor("CALENDAR_SYNC_SETTINGS", "GOOGLE#00000000-0000-4000-8000-000000000001"))
        ?.data ?? "{}"));
    assert.equal(savedSettings.syncFromYear, syncFromYear);
    assert.equal(savedSettings.syncToYear, syncToYear);
    const connectionResponse = await invokeHandler(createEvent({
        path: "/calendar/google/connection",
        groups: ["admin"],
    }));
...
test("loads Google calendar events across all calendars with one API call", async () => {
    ...
    const storedSyncSettings = store.get(keyFor("CALENDAR_SYNC_SETTINGS", "GOOGLE#00000000-0000-4000-8000-000000000001"));
...
test("handles the Google Calendar OAuth callback and redirects back to the app", async () => {
    ...
    assert.match(String(response.body ?? ""), /^$/);
    assert.match(String(response.headers?.Location ?? response.headers?.location), /^https:\/\/app\.example\.com\/calendar\/connect\?calendar-google-status=success&calendar-google-message=Google\+Calendar\+connected\.$/);
    assert.ok(dynamo.commands.some((command) => command.constructor.name === "DeleteCommand"));
});
test("deletes the Google Calendar OAuth state when callback is missing an authorization code", async () => {
    ...
    assert.equal(response.statusCode, 302);
    assert.match(String(response.headers?.Location ?? response.headers?.location), /^https:\/\/app\.example\.com\/calendar\/connect\?calendar-google-status=error&calendar-google-message=Google\+did\+not\+return\+an\+authorization\+code\.$/);
    assert.ok(dynamo.commands.some((command) => command.constructor.name === "DeleteCommand"));
});
test("deletes the Google Calendar OAuth state when token exchange fails", async () => {
    ...
    globalThis.fetch = originalFetch;
    assert.equal(response.statusCode, 302);
    assert.match(String(response.headers?.Location ?? response.headers?.location), /^https:\/\/app\.example\.com\/calendar\/connect\?calendar-google-status=error&calendar-google-message=Bad\+code$/);
    assert.ok(dynamo.commands.some((command) => command.constructor.name === "DeleteCommand"));
});
...
test("imports Unity Excel members using unity sk values and allows shared phones", async () => {
    ...
    const body = parseBody(response.body);
    const putCommands = dynamo.commands.filter((command) => command.constructor.name === "PutCommand");
    const firstImportedData = JSON.parse(String(putCommands[0]?.input?.Item?.data ?? "{}"));
    const secondImportedData = JSON.parse(String(putCommands[1]?.input?.Item?.data ?? "{}"));
    assert.equal(response.statusCode, 200);
    ...
    assert.equal(firstImportedData.notes, [
